  ...
  <div>
    <v-card class="overflow-hidden">
      ...
      >
        ...
        <v-spacer></v-spacer>
        <!-- 此处需要在public/index.html中引入material的css -->
        <v-btn icon>
              ...
              >
                <v-list dense nav class="py-0">
                  <v-list-item two-line :class="miniVariant && 'px-0'">
                    <v-list-item-avatar>
                      <img @click="miniVariant = !miniVariant" :src="user.avatar" />
                    </v-list-item-avatar>

                    <v-list-item-content>
                      <v-list-item-title>
                        <h3 class="gutter">{{ user.username }}</h3>
                      </v-list-item-title>
                  ...
                  <v-divider></v-divider>
                  <!-- 导航部分，遍历items（即定义的固定数据） -->
                  <v-list-item v-for="item in items" :key="item.title" link>
                    <!-- 若遍历出的数据没有子菜单，则只显示一个icon，标题以及url，点击可直接跳转 -->
